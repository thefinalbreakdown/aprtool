...
if uploaded_file:
    ...
    # Auto-detect whether to scale
    sample = df[funding_col].dropna().abs()
    if sample.median() > 1:
        df[funding_col] = df[funding_col] / 100  # it was in percent format

    df = df.dropna(subset=[funding_col])

    # Calculate funding % and APR
